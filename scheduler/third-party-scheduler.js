...
class TrackOffersWorker {
  ...
  async execute() {
    ...
    console.log(
      `Processing third party tracking (${asin}): "${description}" with tier: ${tier}`,
    );
    await this.addDelay();
    const html = await this.browserClient.getPageHTML(asin);
    let offers = Scraper.getThirdPartyOffers({
      html,
      asin,
    });
    ...
    //ONLY FOR THIS TIME
    for (const offer of offers) this.notify(offer);
